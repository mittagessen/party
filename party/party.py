...
from party.modules import PromptCrossAttention, RMSNorm
...
class PartyModel(nn.Module, RecognitionBaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        ds_factors = list(kwargs.get('adapter_ds_factors', [4, 2, 1]))
        ...
        if len(ds_factors) != len(out_indices):
        ...
        if float(kwargs.get('ctc_aux_weight', 0.0)) > 0.0:
            modules['ctc_head'] = nn.Sequential(RMSNorm(decoder_embed_dim, eps=1e-5),
    ...
    def predict_tokens(self,
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            line_features, _ = self.forward_line_features(adapter_output,
