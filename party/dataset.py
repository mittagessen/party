...
def _codepoints_from_metadata(raw_metadata: Optional[dict]) -> Optional[list[int]]:
    if not raw_metadata:
        return None
...
def _max_tokens_from_metadata(raw_metadata: Optional[dict]) -> int:
    if not raw_metadata or b'max_tokens_in_line' not in raw_metadata:
        return 0
...
def _tokenizer_from_table(table) -> CodePointTokenizer:
...
class BinnedBaselineDataset(Dataset):
    ...
    def __init__(self,
        ...
        if all_codepoints:
            ...
            logger.info('Building code point registry from training dataset contents.')
...
class TestBaselineDataset(Dataset):
    ...
    def __init__(self,
        ...
        if all_codepoints:
            ...
            logger.info('Building code point registry from test dataset contents.')
            self.tokenizer = _tokenizer_from_table(self.arrow_table)
